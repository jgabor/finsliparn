...
**Finsliparn** (Swedish: *the honer*) — from *finslipa*, meaning "to hone", "to fine-tune" or "to put the finishing touches on." Swedes use this for final polishing: *"finslipa metoden"* (perfect the method). The definite form *-arn* gives it a craftsman quality, like naming a tool after the artisan who wields it (and just something Jonathan likes to call their random stuff).
...
const tools = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        testCommand: { type: "string", default: "bun test" },
        maxIterations: { type: "number", default: 5 },
        targetScore: { type: "number", default: 1.0 }
      },
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        sessionId: { type: "string" },
        strategy: { type: "string", enum: ["highest_score", "minimal_diff", "balanced"] }
      },
      required: ["sessionId"]
    }
  },
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        sessionId: { type: "string" },
        iteration: { type: "number", description: "Iteration to merge (optional, defaults to winner)" }
      },
      required: ["sessionId"]
    }
  },
...
export const handler: CommandHandler = async (args, context) => {
  ...
  return {
    type: 'message',
    content: `Started refinement session: ${session.id}\n\n` +
...
export const hook: PostToolUseHook = async (event, context) => {
  ...
  if (iteration.score >= activeSession.config.targetScore) {
    return {
      decision: 'inject',
      content: `\n\n✅ **All tests passing!** Score: ${(iteration.score * 100).toFixed(1)}%\n\n` +
  ...
  return {
    ...
    content: `\n\n**Iteration ${iteration.iteration} Results**\n` +
             `Score: ${(iteration.score * 100).toFixed(1)}% (${iteration.testResults.passed}/${iteration.testResults.total} tests)\n\n` +
...
class BunTestRunner implements TestRunner {
  ...
  async detect(cwd: string): Promise<boolean> {
    ...
    return packageJson.scripts?.test?.includes('bun') ?? false;
  }

  async run(cwd: string, options: TestRunOptions): Promise<TestResults> {
    const proc = Bun.spawn(['bun', 'test', '--json'], {
      cwd,
  ...
  parseOutput(stdout: string): TestResults {
...
class WorktreeManager {
  ...
  async merge(worktreePath: string, targetBranch: string = 'main'): Promise<void> {
    const worktreeGit = simpleGit(worktreePath);
...
function calculateScore(
  ...
  const baseScore = testResults.total > 0
...
describe('Session Lifecycle', () => {
  ...
  test('creates session with worktree', async () => {
    ...
    expect(await Bun.file(`${testDir}/.finsliparn/sessions/${session.id}/state.json`).exists()).toBe(true);
  });
});
```
...
- [ ] Worktree manager
- [ ] **Directive Writer & State Management**
- [ ] Test runner abstraction (Bun, Vitest)

### Phase 2: MCP Server

- [ ] MCP server implementation
- [ ] Tool handlers (finslipa_start, **check**, **vote**, complete)
- [ ] Response formatting with `nextSteps`
- [ ] State persistence

### Phase 3: Plugin Integration

- [ ] Command handlers (/finslipa, /finslipa:status)
- [ ] PostToolUse hook for feedback injection
...
- [ ] Score calculation & **Diff Analysis**
...
- [ ] Copilot CLI support (see [spec-copilot.md](spec-copilot.md))
